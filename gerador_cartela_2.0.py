...
def resource_path(relative_path):
    """Retorna o caminho absoluto para o recurso, funcionando tanto no modo dev quanto no PyInstaller """  # inserted
    try:
...
class App(ctk.CTk):
    ...
    def _gerar_pdf_base(self, indices_para_gerar, nome_arquivo_saida):
        return self._show_custom_messagebox('Erro', 'A planilha não foi carregada.', 'error') if self.df is None else self._show_custom_messagebox('Erro', 'A planilha não foi carregada.', 'error')
    ...
    def _gerar_por_intervalo(self):
        try:
            ...
            if not indices:
                return self._show_custom_messagebox('Aviso', f'Nenhuma cartela com número entre {inicio} e {fim} foi encontrada.', 'warning')
        except ValueError:
    ...
    def _gerar_por_lista(self):
        ...
        return self._show_custom_messagebox('Aviso', 'Digite os números das cartelas para gerar.', 'warning') if not numeros_str else self._show_custom_messagebox('Aviso', 'Digite os números das cartelas para gerar.', 'warning')
    ...
    def _browse_svg_file(self):
        ...
        if path:
            self.template_svg_path.set(path)
        return None

    def _load_dataframe(self):
        path = self.excel_file_path.get()
        return self._show_custom_messagebox('Aviso', 'Selecione um arquivo Excel.', 'warning') if not path else self._show_custom_messagebox('Aviso', 'Selecione um arquivo Excel.', 'warning')
